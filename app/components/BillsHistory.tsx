      ...
      <div className="px-4 py-3 space-y-2">
        ...
        <div className="flex items-center justify-between border-t border-slate-100 dark:border-zinc-800 pt-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => isOnline ? onDelete(sale) : alert("Deleting bills requires an internet connection.")}
              className={`flex items-center gap-1 rounded-lg border border-rose-200 bg-rose-50 px-2.5 py-1.5 text-[11px] font-black text-rose-600 hover:bg-rose-100 dark:border-rose-900/50 dark:bg-rose-950/40 dark:text-rose-400 dark:hover:bg-rose-900/60 transition-colors ${
                !isOnline ? "opacity-50 cursor-not-allowed" : ""
              }`}
              title={isOnline ? "Delete this bill" : "Requires internet connection"}
            >
...
export default function BillsHistory({ isOnline, customers, products }: BillsHistoryProps) {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!isOnline) {
      setDeleteError("Deleting bills requires an internet connection.");
      return;
    }
    ...
    try {
      const targetId = deletingSale._id || deletingSale.invoiceNumber;
      const res = await fetch(`/api/sales/${encodeURIComponent(targetId)}`, {
        method: "DELETE",
        headers: {
      ...
      const deletedInvoice = deletingSale.invoiceNumber;
      const deletedMongoId = deletingSale._id;

      // Remove from sales list
      setSales((prev) =>
        prev.filter((s) => s.invoiceNumber !== deletedInvoice && s._id !== deletedMongoId)
      );

      // Close receipt/edit modals if they were showing the deleted sale
      if (viewingSale && (viewingSale.invoiceNumber === deletedInvoice || viewingSale._id === deletedMongoId)) {
        setViewingSale(null);
      }
      if (editingSale && (editingSale.invoiceNumber === deletedInvoice || editingSale._id === deletedMongoId)) {
        setEditingSale(null);
      }
      ...
      console.error("[BillsHistory] handleConfirmDelete error:", err);
    ...
    <div className="mx-auto max-w-4xl space-y-4 sm:space-y-5 px-3 py-4 pb-20 sm:px-6 sm:py-6 sm:pb-24 lg:px-8">
      ...
      {deletingSale && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white p-6 shadow-2xl dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            {/* Header Icon + Title */}
            <div className="flex flex-col items-center text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl mb-2 bg-rose-100 text-rose-600 dark:bg-rose-950/50 dark:text-rose-400">
            ...
            {/* Bill Details Box */}
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 text-xs dark:border-zinc-700 dark:bg-zinc-800 space-y-2">
              <div className="flex justify-between items-center">
                <span className="font-bold text-slate-500 dark:text-zinc-400">Bill Number:</span>
                <span className="font-mono font-black text-slate-900 dark:text-white">{deletingSale.invoiceNumber}</span>
              </div>
              {deletingSale.customerName && (
                <div className="flex justify-between items-center">
                  <span className="font-bold text-slate-500 dark:text-zinc-400">Customer:</span>
                  <span className="font-black text-slate-800 dark:text-zinc-200">{deletingSale.customerName}</span>
                </div>
              )}
              <div className="flex justify-between items-center">
                <span className="font-bold text-slate-500 dark:text-zinc-400">Total Amount:</span>
                <span className="font-black text-slate-900 dark:text-white">Rs. {deletingSale.totalAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-bold text-slate-500 dark:text-zinc-400">Payment:</span>
                <span className="font-bold text-slate-800 dark:text-zinc-200">{pmLabel(deletingSale.paymentMethod)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-bold text-slate-500 dark:text-zinc-400">Date:</span>
                <span className="font-medium text-slate-600 dark:text-zinc-400">{fmtDate(deletingSale.createdAt)}</span>
              </div>
              <p className="text-[11px] font-semibold text-rose-600 dark:text-rose-400 pt-1 border-t border-slate-200 dark:border-zinc-700">
                ⚠ This bill will be permanently removed from MongoDB.
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <button
                ...
                disabled={isDeleting}
                className="flex-1 rounded-xl border-2 border-slate-200 bg-slate-100 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 transition-colors cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isDeleting}
                className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-rose-600 py-2.5 text-sm font-black text-white shadow-lg shadow-rose-500/25 hover:bg-rose-500 disabled:opacity-60 transition-all active:scale-[0.98] cursor-pointer"
              >
                {isDeleting ? (
                  <>
                    <svg className="h-4 w-4 animate-spin text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
